...
### Required Hardware
At least 75 GB Hard drive space
...
### Downloading the Singing Models
Hay Say only downloads talking models for so-vits-svc (3.0 and 4.0) by default. Singing models are available and you can configure Hay Say to download them too by editing the docker-compose.yaml file. You will need to "uncomment" some lines.

To download the so-vits-svc 3.0 singing models, uncomment (remove the hashtag at the start of) the following lines:
```
...
![diagram of Hay Say's networking setup, showing that the main UI container communicates with the AI Architecture containers by sharing files and by using webservice calls to trigger audio generation](documentation images/design diagram.png)
